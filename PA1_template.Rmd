...
```{r load_process, message=FALSE, warning=FALSE}
library(dplyr)
library(ggplot2)
library(reshape2)
library(knitr)
...
```{r make_dataframes, warning=FALSE}
## Make dataframes
by5min <<- activity %>% 
...
#### - The mean number of steps per day is `r round(mean(byday$total.steps))` 

#### - The median number of steps is `r median(byday$total.steps)`


The  histogram for number of steps per day is shown below. 

```{r hist_mean, fig.height=2, fig.width=6}
...
```{r daily_pattern, fig.height=2, fig.width=6,}
ggplot(bytime, aes(x=time, y=mean.steps)) + 
    geom_area(group=1, size=1, fill='navy', alpha=0.7) 
```

#### We see a sharp peak in daily activity in the morning, with the maximum average steps (`r round(max(bytime$mean.steps)) ` in 5 minutes) at `r format(bytime[bytime$mean.steps == max(bytime$mean.steps),]$time, '%H:%M AM')`.

We can also see if the daily pattern differs by the day of the week:

```{r weekday_pattern, fig.height=6, fig.width=6}
...
```{r histogram_missing, fig.height=4, fig.width=6}
...
```{r weekday_weekend, fig.height=4, fig.width=6}
